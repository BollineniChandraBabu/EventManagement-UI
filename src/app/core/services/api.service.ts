...
export class ApiService {
  ...
  eventTypeSeeds(searchKey = ''): Observable<EventTypeSeed[]> {
    return this.requestWithFallback((path) => this.http.get<ApiResponse<EventTypeSeed[]>>(`${environment.apiUrl}${path}`, {
      params: new HttpParams().set('searchKey', searchKey)
    }), this.eventTypeSeedPaths).pipe(map((response) => this.unwrap(response)));
  }

  eventTypeSeedById(id: number): Observable<EventTypeSeed> {
    return this.requestWithFallback((path) => this.http.get<ApiResponse<EventTypeSeed>>(`${environment.apiUrl}${path}/${id}`), this.eventTypeSeedPaths).pipe(
      map((response) => this.unwrap(response))
    );
  }
